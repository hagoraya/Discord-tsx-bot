...
async function getData(url) {
  ...
  await getHTML(url).then((html) => {
    ...
    $('div[data-test="left-summary-table"] > table > tbody', html).each(
      function (i, item) {
        StockData.range52 = $(
          'tr  > td[data-test="FIFTY_TWO_WK_RANGE-value"]',
          item
        ).text();
        StockData.dayRange = $(
          'tr > td[data-test="DAYS_RANGE-value"]',
          item
        ).text();
      }
    );
...
async function startScraper(ticker) {
  ...
  if (EmptyCheck(data)) {
    ...
    if (EmptyCheck(data)) {
      console.log("Stock not found, trying again");
      return null;
    }
  }
